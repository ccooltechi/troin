...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Troin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Troin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Troin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Troin 0.7 datadir."
    ;;
  3)
    echo "Detected Troin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Troin 0.8 datadir."
    ;;
esac
